...
* Initialize Botpress chatbotAdd commentMore actions
 */
...
function loadBotpressScript() {
    // Get Botpress credentials from environment or use default
    const botpressUrl = window.ENV?.BOTPRESS_URL || 'https://cdn.botpress.cloud/webchat/v0/inject.js';
    const botpressId = window.ENV?.BOTPRESS_ID || 'your-botpress-id';
    
    if (!window.botpressWebChat) {
        const script = document.createElement('script');
        script.src = botpressUrl;
...
function openChatbot() {
    if (window.botpressWebChat) {
        ...
        // Fallback: show a simple message if Botpress isn't loaded
